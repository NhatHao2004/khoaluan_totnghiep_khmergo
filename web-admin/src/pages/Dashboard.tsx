        ...
        {isHistoryOpen && (
          <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2rem' }}>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="card" style={{ position: 'relative', width: '100%', maxWidth: '800px', minHeight: '650px', padding: '2.5rem', borderRadius: '32px', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '1rem', overflowX: 'auto', WebkitOverflowScrolling: 'touch' }} className="custom-scrollbar">
                {['all', 'users', 'feedback', 'system'].map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab)} style={{ border: 'none', background: 'transparent', padding: '0.5rem 1rem', borderRadius: '8px', fontSize: 'clamp(0.7rem, 2.5vw, 0.8125rem)', fontWeight: 700, color: activeTab === tab ? 'var(--primary)' : 'var(--text-muted)', cursor: 'pointer', position: 'relative', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>
